...
@section('title', __('Add Dzongkhag'))

@section('content')
    <x-forms.post :action="route('admin.dzongkhag.store')">
        <x-backend.card>
            <x-slot name="header">
            ...
            <x-slot name="body">
                <div x-data="{userType : '{{ $model::TYPE_USER }}'}">
                    ...
                    <div class="form-group row">
                        <label for="cid" class="col-md-2 col-form-label">@lang('CID')</label>

                        <div class="col-md-10">
                            <input type="text" name="cid" class="form-control" placeholder="{{ __('CID') }}" value="{{ old('cid') }}" maxlength="100" required />
                        </div>
                    </div><!--form-group-->
